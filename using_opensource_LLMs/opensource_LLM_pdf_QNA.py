...
class pdfQNA: 
    ...
    def init_embeddings(self):
        ...
        if self.config["embedding"] == EMB_OPENAI_ADA: 
            ...
            if self.embedding is None: 
                self.embedding = pdfQNA.create_instructor_xl()
        elif self.config["embedding"] == EMB_SBERT_MPNET_BASE: 
            # For SBERT
            if self.embedding is None: 
                self.embedding = pdfQNA.create_sbert_mpnet
    ...
    def init_models(self): 
        ...
        if self.config["llm"] == LLM_OPENAI_GPT35: 
            ...
            if self.llm is None: 
                self.llm = pdfQNA.create_flan_t5_small(load_in_8bit=load_in_8bit)
        elif self.config["llm"] == LLM_FLAN_T5_BASE:
            if self.llm is None:
                self.llm = pdfQNA.create_flan_t5_base(load_in_8bit=load_in_8bit)
        elif self.config["llm"] == LLM_FLAN_T5_LARGE:
            if self.llm is None:
                self.llm = pdfQNA.create_flan_t5_large(load_in_8bit=load_in_8bit)
        elif self.config["llm"] == LLM_FLAN_T5_XL:
            if self.llm is None:
                self.llm = pdfQNA.create_flan_t5_xl(load_in_8bit=load_in_8bit)
        elif self.config["llm"] == LLM_FLAN_T5_XXL:
            if self.llm is None:
                self.llm = pdfQNA.create_flan_t5_xxl(load_in_8bit=load_in_8bit)
        elif self.config["llm"] == LLM_FASTCHAT_T5_XL:
            if self.llm is None:
                self.llm = pdfQNA.create_fastchat_t5_xl(load_in_8bit=load_in_8bit)
        elif self.config["llm"] == LLM_FALCON_SMALL:
            if self.llm is None:
                self.llm = pdfQNA.create_falcon_instruct_small(load_in_8bit=load_in_8bit)
